...
export const add = ({
    ...
    next(action);
    if (action.type === ADD) {
        dispatch(RESTAdd(ikeUsuarioFetch, action.body, ADD_SUCCESS, ADD_ERROR, action.token))
    }
};
...
export const update = ({
    ...
    if (action.type === UPDATE) {
        dispatch(RESTUpdate(ikeUsuarioFetch, action.id, action.body, UPADTE_SUCCESS, UPDATE_ERROR, action.token))
    }
};
...
export const patch = ({
    ...
    if (action.type === PATCH) {
        dispatch(RESTPatch(ikeUsuarioFetch, action.id, action.body, PATCH_SUCCESS, PATCH_ERROR, action.token))
    }
};
...
export const remove = ({
    ...
    if (action.type === REMOVE) {
        dispatch(RESTDelete(ikeUsuarioFetch, action.id, REMOVE_SUCCESS, REMOVE_ERROR, action.token))
    }
};
