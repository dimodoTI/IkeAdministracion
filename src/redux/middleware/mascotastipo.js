...
export const add = ({
    ...
    next(action);
    if (action.type === ADD) {
        dispatch(RESTAdd(ikeMascotasTipo, action.body, ADD_SUCCESS, ADD_ERROR))
    }
};
...
export const update = ({
    ...
    if (action.type === UPDATE) {
        dispatch(RESTUpdate(ikeMascotasTipo, action.id, action.body, UPDATE_SUCCESS, UPDATE_ERROR))
    }
};
...
export const patch = ({
    ...
    if (action.type === PATCH) {
        dispatch(RESTPatch(ikeMascotasTipo, action.id, action.body, PATCH_SUCCESS, PATCH_ERROR))
    }
};
...
export const remove = ({
    ...
    if (action.type === REMOVE) {
        dispatch(RESTDelete(ikeMascotasTipo, action.id, REMOVE_SUCCESS, REMOVE_ERROR))
    }
};
